...
router.post("/", checkloginuser, async function (req, res, next) {
  ...
  let pay_details = req.body.pay_details;
  let pay = req.body.pay;
  const payrole = await new payrolemodels({
    employee_name: pay_details,
    pay_role_statu: pay,
  });
  const savedRole = await payrole.save()
  res.render("pay_role", {
    title: "Employee managment system",
    loginUser: loginUser,
    recods: savedRole,
    success: "pay role inserted successfully",
  });
});
